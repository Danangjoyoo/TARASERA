...
class haarCC(detClasses):
    def __init__(self,name, cam_id):
        ...
        self.tempVal = [[],[]]
        self.dragStat = False
        self.parameters = [[1.2, 3, 1.07, 7, 3, 0.6, 5], [0, 0, 1, 1]]
        self.cursorControl = 0
        self.showWin = 2
        self.showBorder = 2
    ...
    def detectObject(self):
        ...
        for i in enumerate(fist_cont):
            if i[0] < 2:
                ...
                cv2.putText(img, f'{i[0]+1}', (int((x+(w/2))*0.90), int((y+(h/2))*1.1)), cv2.FONT_HERSHEY_COMPLEX, 1, (10, 255, 10), 2)
        if not fist_cont:
            ...
            for i in enumerate(palm_cont):
                if i[0] < 2:
                    ...
                    cv2.putText(img, f'{i[0]+1}', (x + 10, y + 20), cv2.FONT_HERSHEY_COMPLEX, 1, (10, 10, 255), 2)
    ...
    def getStat(self):
        if len(self.palm) > 0 and len(self.fist) == 0:
            self.dragStat = False
        elif len(self.fist) > 0 and len(self.palm) == 0:
            self.dragStat = True
    ...
    def inputParameters(self, params):
        ...
        if params:
            ...
            self.cursorControl = params[1][1]
            self.showWin = params[1][2]
            self.showBorder = params[1][3]
